...
class ChartTool extends Component {
  constructor(props) {
    super(props);
    this.state = {
      hoverLoc: null,
  ...
  getY(){
    ...
    return {
      ...
      /* p.prediction < p.y ? (p.prediction < min ? p.prediction : min) : (p.y < min ? p.y : min), data[0].y), */
      /*  p.prediction >= p.y ? (p.prediction < max ? p.prediction : max) : */
    }
  }
  // GET SVG COORDINATES
  getSvgX(x) {
  ...
  // BUILD SVG Predict PATH
  makePredictPath() {
    const {data} = this.props;
    const {color} = '#ffa500';
    let pathD = "M " + this.getSvgX(data[0].x) + " " + this.getSvgY(data[0].prediction) + " ";
  
    pathD += data.map((point, i) => {
      return "L " + this.getSvgX(point.x) + " " + this.getSvgY(point.prediction) + " ";
    }).join("");
  
    return (
      <path className="linechart_path_prediction" d={pathD} style={{stroke: color}} />
    );
  }
  // BUILD SHADED AREA
  makeArea() {
    ...
    return (
      <g className="linechart_axis">
        <line
          ...
          x2={this.getSvgX(x.max) + yLabelSize} y2={this.getSvgY(y.min)}
          strokeDasharray="5" />
        <line
          x1={this.getSvgX(x.min) - yLabelSize} y1={this.getSvgY(y.max)}
          x2={this.getSvgX(x.max) + yLabelSize} y2={this.getSvgY(y.max)}
          strokeDasharray="5" />
      </g>
    );
  }
  ...
  getCoords(e){
    ...
    data.map((point, i) => {
      svgData.push({
        ...
        svgY: this.getSvgY(point.prediction),
        d: point.d,
        p: point.p,
